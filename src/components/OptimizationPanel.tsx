...
export default function OptimizationPanel({pattern,inventory,onApply,onToast}){
 ...
 function run(){if(!pattern)return onToast?.('先创建图纸','bad');setBusy(true);setTimeout(()=>{try{const r=mode==='zero'?optimizeToInventory(pattern,inventory):optimizeBalanced(pattern,inventory,{maxDeltaE:Number(maxDeltaE)||15});setResult(r);if(!r.ok)onToast?.(r.reason||'无法生成方案','bad');else onToast?.('优化完成','ok')}catch(e){console.error(e);onToast?.('优化失败','bad')}finally{setBusy(false)}},20)}
 return <div className="card"><div className="card-hd"><div><h2>库存约束改图</h2><p>优先用你手上的豆</p></div><span className="badge">ΔE00</span></div><div className="card-bd">
   <div className="seg"><button className={mode==='zero'?'active':''} onClick={()=>setMode('zero')}>0元版</button><button className={mode==='balanced'?'active':''} onClick={()=>setMode('balanced')}>平衡版</button></div>
   ...
   {result&&!result.ok&&<div className="callout danger">{result.reason}</div>}
   {pattern&&<div className="mini-note">当前作品 {total.toLocaleString()} 颗。0元版要求总库存足够；平衡版允许少量补豆来保护明显色差。</div>}
 </div></div>
}
